...
class BinaryConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, bias=False, resid_attn=None, bin_attn=None):
    ...
    def forward(self, x):
        resid_attn = x
        if self.resid_attn is not None:
            resid_attn = self.resid_attn(x)
        ...
        bin_attn = out
        if self.bin_attn is not None:
            bin_attn = self.bin_attn(out)        

        out = self.a1 * resid_attn + self.a2 * bin_attn 
        return out
